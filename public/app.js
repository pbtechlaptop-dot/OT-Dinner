...
function summarySystemOptionKeys(foodLabel) {
  ...
  if (food && Array.isArray(food.optionGroups)) {
    food.optionGroups.forEach(group => {
      (group.choices || []).map(normalizeOptionChoice).filter(Boolean).forEach(choice => {
        [choice.label, formatOptionLabel(choice.label, 0), storedOptionChineseLabel(choice.label), extractEnglishOptionLabel(choice.label)]
          .forEach(addKey);
      });
    });
  }
...
function renderStaffFoodSummary(orders) {
  ...
  return Object.entries(byDept).map(([dept, rows]) => {
    ...
    const lines = Object.values(groupedRows).map(row => {
      const suffix = row.count > 1 ? ` ${t('xLabel')} ${row.count}` : '';
